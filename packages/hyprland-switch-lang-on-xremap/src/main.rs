...
fn main() -> hyprland::Result<()> {
    ...
    // Very naive:
    // 1. Relies on not documented fact that xremap creates a keyboard with the name "xremap-pid=$PID"
    // 2. Relies on the fact that one and only one xremap exists
    let binding = devices.keyboards.iter().filter(|device| device.name.starts_with("xremap-pid")).collect::<Vec<_>>();
