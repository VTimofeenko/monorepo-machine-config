...
let
  ...
  inherit (lib) getExe;
  # NOTE: a bit naive way to detect if imported in home-manager
  homeDir = if config ? home then config.home.homeDirectory else config.users.users.spacecadet.home;
  profileDir = if config ? home then config.home.profileDirectory else "${homeDir}/.nix-profile";
in
{
  ...
  programs.zsh = {
    ...
    initExtra =
      ...
      ''
        ...
        l:|=* r:|=*' 'm:{[:lower:][:upper:]}={[:upper:][:lower:]} l:|=* r:|=*'
    ...
    shellAliases = {
      ...
      nvim = "$EDITOR";
      vim = "$EDITOR";
      ls = "${getExe pkgs-unstable.eza} -h --group-directories-first --icons";
      l = "ls";
      ll = "ls -l";
      la = "ls -al";
  ...
  programs.direnv = {
    ...
    config.whitelist.prefix = [ "${homeDir}/code" ];
  };
  programs.starship = {
    enable = true;
