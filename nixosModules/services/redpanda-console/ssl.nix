/**
  Nginx module that configures listening only on management network.

  Another file like this is in `pgadmin` service.
*/

{ config, ... }:
let
  inherit (config) my-data;
  srvName = "redpanda-console";
...
in
{
  ...
  services.nginx = {
    ...
    virtualHosts.${(my-data.lib.getService srvName).fqdn} = {
      ...
      '';
      locations."/" = {
        proxyPass = "http://localhost:${toString config.services.redpanda-console.port}";
