...
let
  ...
  components = map
    ...
    }) [ "services" "settings" "bookmarks" "widgets" ];
in
{
  /* Enable the service */
  ...
  systemd.services.homepage-dashboard = {
    serviceConfig = {
      ...
      ExecStartPost =
        map
          (component: "${pkgs.coreutils}/bin/ln --symbolic --force ${component.pkg}/share/${component.name}.yaml ${srvDataDir}")
          components;
      ...
      LoadCredential = lib.mapAttrsFlatten
        (name: _:
          "${name}:${config.age.secrets.${name}.path}")
        srv.secrets;
    };
    ...
    environment = lib.mapAttrs'
      (name: _:
        lib.nameValuePair
          ("HOMEPAGE_FILE_" + (lib.toUpper name))
          "%d/${name}")
      srv.secrets;
  };
