    ...
    let
      ...
      mergeOne = serviceName:
        let
          ...
          # Extract the manifest attrset and add auto-assembled .default
          manifestData = evaluated.config.manifest;

          # Auto-assemble .default field from manifest components
          # This mimics the current mkManifest logic

          # Collected modules from all sources
          ...
          endpointsModule =
            if manifestData.endpoints ? impl then
              manifestData.endpoints.impl (builtins.removeAttrs manifestData.endpoints ["impl"])
            else
              null;
