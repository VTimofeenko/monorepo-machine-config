...
#+DATE: <2022-11-09 Wed>
#+TAGS: { fix(b) feat(f) doc(d) chore(c) to_think(t) } 2305
...
Emacs run ~nix fmt~ on save and tangle, so however the code is formatted in the ~project.org~ file -- it comes out linted.
...
Useable as ~./homeConfigurations/zsh~.
...
****** TODO Add noti for async notifications :feat:
...
****** DONE Add system-wide completions :feat:
CLOSED: [2023-05-15 Mon 07:44]
...
The deck profile is imported into flake [[(deck-hm-import)][here]].
...
****** TODO Add nixGL to fix kitty :fix:
...
#+end_src
****** TODO check if this module is needed or it can be removed in favor of home manager module :chore:
***** Zathura
...
***** TODO Add firefox to home-manager managed apps :feat:
...
#+end_src
***** TODO Add gh to homeManager :feat:
...
eww is not enabled yet as turns out I am pretty OK without any bars.

******** CSS
#+begin_src nix :tangle modules/hyprland/eww/css.nix
{
  eww-scss =
    ''
      ,* {
...
Taken from [[https://wiki.hyprland.org/Useful-Utilities/Status-Bars/#configuration][Hyprland wiki]]

#+begin_src nix :tangle modules/hyprland/eww/scripts.nix
pkgs:
{
...
********* TODO if grows - rewrite using keymap :chore:
...
******** TODO Basic date :feat:

******** TODO Basic workspace number :feat:

******** TODO [#A] Rethink the future state -- maybe make it a modal only thing? :feat:

- Appear only on super
- Custom colors
- Embed clock into desktop
- Add which-key mode?
...
******** TODO Style wofi :feat:2305:

******** TODO Import wofi in hyprland module :2305:chore:

******* Xremap

Module that configures system-wide key remapping. Some of the maps are application-specific

#+begin_src nix :tangle modules/xremap/default.nix
{ pkgs, config, lib, ... }:
{
  services.xremap =
    {
...
******* TODO Global "hyper+e" = emacs :feat:

******* TODO Add notification to language switch setup :feat:

******* TODO Global "hyper-b" = browser :feat:

******* TODO focus mode_toggle to switch focus between floating windows and tiled ones :feat:

******* TODO consider super+ctrl+grave for terminal :feat:
...
******* TODO Port over F-and-XF86-whatever bindings :feat:
...
******* TODO port over workspace edit/renumber/rename mode :feat:

******* TODO Port over the resize mode :feat:

******* TODO Port the shutdown mode :feat:

******* TODO Consider a new launcher :feat:

******* TODO Fix the xremap ordering for hyprland :feat:

******* TODO Custom tools [0/3] :feat:
...
#+begin_src nix :tangle modules/hyprland/system.nix
...
This is a holding headline for various projects to change big parts of the config
...
*** TODO Merge doom config here

Doom files could be tangled from a subdirectory.

**** TODO Re-evaluate current flake approach :to_think:

Currently the whole flake needs to rebuild if a single file changes. This slows down development, so I will need to rethink this approach. IIRC the author of doom-emacs had a flake.

**** TODO Set up emacs to leave machine-local files (custom.el) outside Nix store :feat:

Stuff like trusted ~.dir-locals.el~ cannot be recorded in the store.
