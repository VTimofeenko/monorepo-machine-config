...
let
  ...
  mkServiceRecord =
    serviceName: manifest: instanceName:
    let
      ...
      hasHttpsEndpoint = manifest.endpoints ? web && (manifest.endpoints.web.protocol or null) == "https";

      # Determine record type and target
      recordType = if hasHttpsEndpoint then "CNAME" else "A";
      recordTarget =
        if hasHttpsEndpoint then
      ...
      domainName =
        let
          service = instanceName |> lib.homelab.services.get;
        in
        if service |> builtins.hasAttr "domain" then service.domain else instanceName;

      # Create the DNS record
      dnsRecord = "${domainName} IN ${recordType} ${recordTarget}${
        if recordType == "CNAME" then "." else ""
      }";

      # Create TXT location record
      hostName = instanceName |> lib.homelab.getServiceHost;
      txtRecord = "_i.${domainName} IN TXT @ ${hostName}";
    in
    if hasNetworkAccess' then { inherit dnsRecord txtRecord; } else null; # Skip localhost-only services
  ...
  mkServiceRecords =
    ...
    in
    instances |> map (instanceName: mkServiceRecord serviceName manifest instanceName);
  ...
  zoneData = (dnsRecords |> lib.concatLines) + "\n\n" + (txtRecords |> lib.concatLines) + "\n";
in
{
